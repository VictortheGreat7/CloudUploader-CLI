...
function prompt_user_action() {
 local filename="$1"
 while true; do
  read -p "File '$filename' already exists. Choose an action: (o)verwrite, (s)kip, (r)ename: " choice
  case "$choice" in
   o|O) echo "overwrite"; return 0 ;;
   s|S) echo "skip"; return 0 ;;
   r|R) echo "rename"; return 0 ;;
   *) echo "Invalid choice. Please enter 'o' for overwrite, 's' for skip, or 'r' for rename." ;;
  esac
 done
}
...
for filename in "${filenames[@]}"; do
 ...
 file_exists=$(az storage blob exists --account-name "$storage_account_name" --container-name "$target_container" --name "$filename" --query "exists" --output tsv)

 if [ "$file_exists" == "True" ]; then
  # Prompt the user for action
  action=$(prompt_user_action "$filename")
  ...
  case "$action" in
   ...
   skip)
    ...
    ;;
   rename)
    read -p "Enter new name for '$filename': " new_filename
